...
config = (
    ...
    "  nominatorCount: %d,    // distinct nominators, counted over the RDF graph\n"
    "};\n" % (json.dumps(args.data_url or ("data/" + args.data_file)),
               json.dumps(DATA["featured"]), DATA["nominatorCount"])
)
...
environmental science datasets that researchers say reshaped their field. Browse %d datasets
across %d discipline groups, nominated by %d researchers.">
<meta property="og:title" content="Impactful Datasets — AGU">
<meta property="og:description" content="%d datasets nominated by the Earth and space science community.">
...
<link rel="alternate" type="application/ld+json" href="%s" title="Collection metadata (schema.org)">
</head>''' % (len(DATA["datasets"]), len(payload["themes"]), DATA["nominatorCount"],
              len(DATA["datasets"]), args.data_url or ("data/" + args.data_file))
